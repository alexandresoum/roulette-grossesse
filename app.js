...
let mode="ddr", selected=null, dragging=false,lastAngle=0;
const mainDate=$("#mainDate"),targetDate=$("#targetDate");
...
const reps=[
 ["Début fenêtre T1","11 SA",77],["Échographie T1","11 SA → 13 SA + 6 j",77],
 ["Fin fenêtre T1","13 SA + 6 j",97],["Échographie T2","≈ 22 SA",154],
 ["Échographie T3","≈ 32 SA",224],["Début du terme","37 SA",259],
 ["39 SA","39 SA",273],["DPA","41 SA",287]
];
function render(){
 let start=ddr();if(!start)return;
 let today=new Date();today.setHours(12,0,0,0), dpa=add(start,287), shown=selected||today;
 $("#dpa").textContent=fmt(dpa,true);$("#dpa2").textContent=fmt(dpa);
 $("#term").textContent=age(shown,start);$("#selectedDate").textContent=fmt(shown);
 ...
 let source=mode==="ddr"?`DDR du ${fmt(start,true)}`:`DDG du ${fmt(add(start,14),true)}`;
 let text=`${source} — terme ce jour : ${age(today,start)} — DPA : ${fmt(dpa,true)}.`;
...
$("#todayBtn").onclick=()=>{selected=null;render()};
...
$("#wheel").addEventListener("pointerdown",e=>{dragging=true;lastAngle=angle(e);$("#wheel").setPointerCapture(e.pointerId)});
$("#wheel").addEventListener("pointermove",e=>{if(!dragging)return;let a=angle(e),delta=a-lastAngle;if(delta>180)delta-=360;if(delta<-180)delta+=360;lastAngle=a;let start=ddr();if(!start)return;let base=selected||new Date();selected=add(base,Math.round(delta/3));let min=add(start,-14),max=add(start,315);if(selected<min)selected=min;if(selected>max)selected=max;render()});
